...
for x in tqdm(clean_data):
    gain = np.random.uniform(1.0, 3.0)
    y = np.tanh(gain * x)

    gain_data.append(np.ones_like(x) * gain)
    dist_data.append(y)
...
freqs, y_fft = plot_fft(dist_data[idx], FS)
...
NUM_TRAIN = 45
NUM_VAL = 5
x_data = np.stack((clean_data, gain_data), axis=1)
print(x_data.shape)
...
plt.plot(dist_data[idx], 'c', label='Actual')
...
freqs, target_fft = plot_fft(dist_data[idx], FS)
